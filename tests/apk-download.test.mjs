...
const RELEASE_TAG = "android-preview-v6";
...
test("Android preview download is an accessible link with disclosure and analytics", () => {
  ...
  assert.match(
    source,
    /<p id=["']android-preview-description["']> Preview v6 · Debug-signed\. Uninstall preview v5 first\. The waypoint keeps its original position with a visible themed outline\. <\/p>/,
  );
  assert.match(
    anchor,
    /trackEvent\(["']android_app_download["'],\s*\{\s*build:\s*["']preview_v6_debug_signed["'],\s*source:\s*["']account_settings["'],?\s*\}\)/,
  );
});
